#!/usr/bin/env bash
# Cross-compile Work and Overwork for Ableton Move (aarch64 Linux) and package
# clean GNU-tar archives. No hardware is touched by this script.
#
# Both builds compile from the SAME src/work_core.c — one engine, two
...
rm -rf build/modules
mkdir -p build/modules/audio_fx/work build/modules/overtake/overwork
...
docker run --rm -v "$PWD":/w -w /w "$IMAGE" bash -c "
    ...
    file build/modules/audio_fx/work/work.so build/modules/overtake/overwork/dsp.so
...
echo "Built: build/work-module.tar.gz, build/overwork-module.tar.gz"
